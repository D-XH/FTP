...
int getToken(char* username, time_t login_time, char* token){
    ...
    char tmp_key[1024] = {0};
    sprintf(tmp_key, "%s time:%ld", username, login_time);
    params.secret_key = (unsigned char*)tmp_key;
...
int valToken(char* username, time_t login_time, char* token){
    char real_token[256] = {0};
    getToken(username, login_time, real_token);
    if(strncmp(real_token, token, sizeof(real_token)) == 0){
        printf("val successful!\n");
        return 0;
    }else{
        printf("val failed!\n");
        return 1;
